...
function CreateListing() {
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto mb-9'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
            <div className='flex flex-col gap-4 flex-1'>
                ...
                <div className='flex flex-wrap gap-8'>
                    ...
                    <div className='flex items-center gap-2'>
                        <input className='p-3 border border-gray-300 rounded-lg' type="text" id="regularPrice" min='5000' size={12} required onChange={handleChange} value={formData.regularPrice} />
                    ...
                    {formData.offer && (
                    <div className='flex items-center gap-2'>
                        <input className='p-3 border border-gray-300 rounded-lg' type="text" id="discountPrice" min='0' max='10000' size={12}  required onChange={handleChange} value={formData.discountPrice} />
                        <div className='flex flex-col items-center'>
                            <p>Discount Price</p>
                            <span className='text-xs'>{formData.type === 'rent' ? '(₹ / month)' : ''}</span>
                        </div>
                ...
                <p className='text-red-700 text-sm'>{imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length>0 && formData.imageUrls.map((url,index)=>(
                        <div key={url} className='flex justify-between p-3 border items-center'>
                            ...
                            <button type='button' onClick={()=>handleRemoveImage(index)} className='p-3 text-red-700'><i class="fa-solid fa-trash"></i></button>
                        </div>
                    ))
                }
                <button disabled={loading || uploading} className='bg-green-700 text-white text-center rounded-lg p-3 uppercase hover:opacity-95 disabled:opacity-80'>
